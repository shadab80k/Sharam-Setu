...
export function WorkerProfileModal({
  ...
  const handleShortlist = () => {
    ...
    pushToast("success", `${user.name} has been added to your shortlist!`);
    onClose();
  };

  const handleHire = () => {
    onHire?.(worker.userId);
    pushToast("success", `Hiring invitation sent to ${user.name}!`);
    onClose();
  };

  const handleContact = () => {
    pushToast("info", `Calling ${user.name} (${user.phone})…`);
  };
    ...
    <Modal open={open} onClose={onClose} title="Worker Profile" size="lg">
      <div className="space-y-5">
        ...
        <div className="grid sm:grid-cols-2 gap-4">
          ...
          <div className="p-4 rounded-xl border border-gray-200 bg-white space-y-3">
            <div className="text-xs text-gray-600 font-medium flex items-center gap-1">
              <Briefcase className="h-3.5 w-3.5 text-gray-500" /> Verified Skills
            </div>
        ...
        <div className="flex items-center gap-3 pt-3 border-t border-gray-200 flex-wrap">
          ...
          <Button variant="secondary" size="md" onClick={handleShortlist}>
            Shortlist
          </Button>
          <Button variant="primary" size="md" onClick={handleHire}>
            Hire Worker
          </Button>
        </div>
      </div>
